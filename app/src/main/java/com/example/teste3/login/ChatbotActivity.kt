...
class ChatbotActivity : AppCompatActivity() {
        ...
        Firebase.ai(backend = GenerativeBackend.googleAI())
            .generativeModel(
                ...
                systemInstruction = com.google.firebase.ai.type.content {
                    text("Você é uma bibliotecária virtual da biblioteca da Unifor. " +
                            "Responda apenas perguntas relacionadas à biblioteca, livros, " +
                            "reservas de salas e serviços da biblioteca. " +
                            "Seja simpática e objetiva. Responda em português. " +
                            "Não use formatação Markdown como asteriscos ou hashtags.")
                }
            )
    }
    ...
    private fun responderBot(pergunta: String) {
        ...
        val posDigitando = mensagens.size - 1

        CoroutineScope(Dispatchers.IO).launch {
            try {
                val response = chat.sendMessage(pergunta)
                val resposta = response.text ?: "Não consegui gerar uma resposta."

                // ✅ Remove formatação Markdown
                val respostaLimpa = resposta
                    .replace("**", "")
                    .replace("*", "")
                    .replace("##", "")
                    .replace("#", "")
                ...
                withContext(Dispatchers.Main) {
                    mensagens[posDigitando] = Mensagem(respostaLimpa, Mensagem.TIPO_BOT)
                    adapter.notifyItemChanged(posDigitando)
                    binding.rvMensagens.scrollToPosition(mensagens.size - 1)
                }
            } catch (e: Exception) {
                withContext(Dispatchers.Main) {
                    mensagens[posDigitando] = Mensagem(
